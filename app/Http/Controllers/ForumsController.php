...
class ForumsController extends Controller
{
    public function index()
    {
        ...
        switch (request('filter')) {
             case 'me':
                ...
                $results->setPath('/forum?filter=me');
                 break;
            case "answered":
                $answered = [];
                ...
                $results->setPath('forum?filter=answered');
                break;
             case "unanswered":
                $unanswered = [];
                foreach(Discussion::all() as $d)
                ...
                $results->setPath('forum?filter=unanswered');
                break;
             default:
    ...
    return view('channel')->with('discussions',$channel->discussions()->paginate(1));
    }
}
